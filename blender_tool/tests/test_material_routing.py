...
Pure stdlib (no oodle, no bpy). Every number here is either read out of the
engine's own shader maths or decoded from shipped archive bytes. The
shipped-byte facts are embedded as literals so the suite runs on a clean
checkout — `blender_tool/exports/` is gitignored — and the optional sweep over
`exports/fixtures_mat/` skips itself when those packages are absent.
"""
...
# ---------------------------------------------------------------------------
# Shipped fixture data, decoded from archive 0703fd2acd5803e9
# ---------------------------------------------------------------------------
...
# unconditionally made it 12.5x too dim.
...
# Observed (mattype, blendmode) joint distribution over the 51 fixture packages.
OBSERVED_MODE_PAIRS = {
...
def test_new_role_preimages_are_real():
    """`layer0_alpha_map` / `layer0_secondary_emissive_map` (findings §3, 9 rows each)."""
    expected = {
...
def test_opacity_map_is_transmission_not_alpha():
    """The engine does `color.rgb += background * opacity` — a float3 tint."""
...
# ---------------------------------------------------------------------------
# 3. The alpha chain, as the engine multiplies its terms together
# ---------------------------------------------------------------------------
...
def test_alpha_source_base_color_alpha():
    """The engine's `alpha = composite_diffuse.w * vertexcolor.w`."""
    spec = mat.build_material_spec(
...
def test_bridge_material_routes_layer1_emissive_and_composite_specular():
    """Before 0.3.0 these two came back UNROUTED on the reference material."""
    spec = _bridge_spec()
    assert spec["unrouted_roles"] == []
...
def test_fixture_corpus_routes_every_known_role():
    ...
    # measured counts, locked so a routing regression is visible
...
def test_emissive_intensity_reads_a2_layers_when_present():
    """The scalar decoder may emit a per-layer `layers[]`; it wins over the raw
    hash lookup."""
...
def test_specular_map_is_not_base_color():
    """8 corpus materials carried one; none of them carried a diffuse."""
...
def test_specular_map_packing_is_fresnel_scaled_not_alpha_scaled():
    """`layer0_specular_map` is the SAME slot as `composite_specular.xyz` (both
    land in `layers[i].specalbedo[0]`, which is F0) but a DIFFERENT scale.

    The engine computes
        specalbedo[0] = enable_specular * specular_tint_color * specular_map *
                        fresnel
    with `specintensity[0] = fresnel` -- so the scale is the material scalar
    `fresnel` (authored 0.010), NOT the map's own alpha.
    """
...
def test_roughness_is_sqrt_and_ao_channel():
    """sqrtroughness[0] = components.x; the engine's own roughness is x^2.

    ⚠ `roughness_is_sqrt` describes the TEXEL SPACE, not a Blender conversion.
    Blender's GGX alpha is `Roughness^2` and the engine's is `sqrtroughness^2`,
    so the Blender socket takes components.x RAW. Squaring it there is a bug --
    guarded in tests/blender_material_probe.py.
    """
    spec = mat.build_material_spec(
...
def test_alpha_mode_hints():
    ...
    assert mat.alpha_mode_for("layer0_composite_components") == "NONE"
...
def test_straight_alpha_is_never_auto_selected():
    ...
    are shader-permutation bits and are NOT on disk (findings §2c)."""
...
def test_works_with_an_empty_scalar_dict():
    """The scalar decoder's extended keys are all optional — nothing here may be
    required."""
